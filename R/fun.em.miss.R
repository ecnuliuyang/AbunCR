...
el.miss <- function(x, z.mat, d, K, beta.init, level){
  ...
  if(p == 2){
    ...
    if(sum(mk!=0) == K*2) se <- sqrt(sig2.mele.g(z.mat, d.obs, d.mis, x.obs, x.mis, mk, nu.est, beta.est, lam.est, alpha.est, prob.est))[1:(p+1)]
    if(sum(mk!=0) < K*2) se <- sqrt(sig2.mele.s(z.mat, d.obs, d.mis, x.obs, x.mis, mk, nu.est, beta.est, lam.est, alpha.est, prob.est))[1:(p+1)]
...
sig2.mele.s <- function(z.mat, d.obs, d.mis, x.obs, x.mis, mk, nu.est, beta.est, lam.est, alpha.est, prob.est){
...
sig2.mele.g <- function(z.mat, d.obs, d.mis, x.obs, x.mis, mk, nu.est, beta.est, lam.est, alpha.est, prob.est){
  # pars <- est_ours
  nu <- nu.est
  beta <- beta.est
  g.beta <- plogis(z.mat%*%beta)
